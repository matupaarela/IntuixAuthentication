# 0004-auth-mfa - Implementation Plan

## Architecture Design

```
MfaController
  → POST /api/0004-auth-mfa/enable → MfaEnableCommand
  → POST /api/0004-auth-mfa/verify-setup → MfaVerifySetupCommand
  → POST /api/0004-auth-mfa/disable → MfaDisableCommand
  → GET /api/0004-auth-mfa/status → MfaGetStatusQuery
```
